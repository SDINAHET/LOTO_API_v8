...
(function () {
  ...
  // }
    ...
    /* =========================================================
    Fetch API wrapper : CSRF + cookies + refresh-on-401
  ========================================================= */
  ...
  function isApiUrl(input) {
    ...
    // cas 1: url absolue vers ton backend
    if (url.startsWith(base)) return true;

    // cas 2: url relative "/api/..." (si tu fais fetch("/api/...") côté front)
    if (url.startsWith("/api/")) return true;

    return false;
  }

  async function refreshAccessTokenWithOriginalFetch(originalFetch, baseUrl) {
    const base = baseUrl || getActiveBase();
    const opts = await withCsrfHeaders(
  ...
  async function apiFetch(input, init = {}) {
    const base = getActiveBase();
    const originalFetch = window.__ORIGINAL_FETCH__ || window.fetch;
    ...
    // Si plusieurs requêtes 401 en même temps -> une seule fait refresh, les autres attendent
    if (isRefreshing) {
    ...
    isRefreshing = true;
    try {
      const ok = await refreshAccessTokenWithOriginalFetch(originalFetch, base);
      resolveRefresh(ok);
      if (!ok) return res1;
  ...
  // Patch global fetch : si c’est API => apiFetch, sinon fetch normal
  function setupFetchFastPatch() {
    ...
    window.fetch = function (input, init) {
      ...
      // non-API: juste cookies + CSRF si besoin
  ...
  function setupAxiosFastPatch() {
    ...
    // CSRF sur méthodes mutantes (ton code OK)
    ...
    axios.interceptors.response.use(
      ...
      async (err) => {
        ...
        try {
          const base = getActiveBase();
          // refresh via fetch (déjà CSRF ok) ou axios:
          const ok = await refreshAccessToken(base);
          axResolve(ok);
